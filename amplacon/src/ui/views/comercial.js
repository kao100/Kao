...
export async function telaVendedor({ params, query }) {
  ...
  return h('div.empilha', { style: { gap: '14px' } },
    ...
    secao('Notas fiscais',
      exportadores(() => ({
        ...
        colunas: [
          ...
          { header: 'Origem do vendedor', key: 'vendedorOrigem' },
        ],
      ...
      card(null, null, tabela({
        colunas: [
          ...
          { header: 'Valor', key: 'valor', tipo: 'money', alinhar: 'direita' },
          {
            ...
            key: 'vendedorOrigem',
            formatar: (v) => ({
              nf: 'relatório', pedido: 'pedido', 'pedido-nf': 'pedido→NF', manual: 'definido à mão',
            }[v] || '—'),
          },
        ],
        linhas: detalhe.notas,
    ...
    aviso('A coluna "Vendedor via" mostra como o app chegou até você: pelo relatório de NFs, '
      + 'pelo pedido, ou porque alguém definiu à mão. Nada é atribuído por semelhança.', 'info'));
}
