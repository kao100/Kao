...
export async function telaAjustes() {
  ...
  return h('div.empilha', { style: { gap: '14px' } },
    ...
    h('p.mini.muted.centro', { style: { padding: '10px 0 20px' } },
      'AMPLACON — gestão administrativa · dados locais no aparelho'));
}
...
async function exportarBackup() {
  ...
  const blob = new Blob([JSON.stringify({ app: 'amplacon', versao: 1, em: Date.now(), dados }, null, 1)], { type: 'application/json' });
  download(`amplacon_backup_${today()}.json`, blob);
  ok('Backup exportado.');
}
...
function importarBackup() {
  ...
  input.onchange = async () => {
    ...
    try {
      ...
      if (conteudo.app !== 'amplacon') throw new Error('Este arquivo não é um backup do aplicativo.');
